...
CREATE TABLE IF NOT EXISTS project (
  ...
  last_push_at  TEXT,
  push_status   TEXT,
  -- Sync remoto (pull): mismo patrón que push. El humano pide "traer de remoto e
  -- integrar cambios locales" desde la UI; el worker lo ejecuta y reporta.
  pull_requested INTEGER NOT NULL DEFAULT 0,
  last_pull_at  TEXT,
  pull_status   TEXT,
  last_seen     TEXT,
...
CREATE TABLE IF NOT EXISTS project_branch (
  ...
  -- Cómo entrega el código a esta rama (cada repo diseña su proceso a prod):
  --   'push'  → push directo HEAD:branch (fast-forward de commits nuevos)
  --   'merge' → merge de promote_from → branch, después push
  --   'pr'    → abrir PR promote_from → branch (gh pr create) y mergear
  -- promote_from es la rama origen para merge/pr (ej. develop). promote_notes
  -- son instrucciones libres que el worker sigue al pie (pasos extra, checks).
  promote_strategy TEXT NOT NULL DEFAULT 'push',
  promote_from     TEXT,
  promote_notes    TEXT,
  created_at TEXT NOT NULL DEFAULT (datetime('now')),
...
CREATE TABLE IF NOT EXISTS task (
  ...
  commit_hash    TEXT,
  committed_at   TEXT,
  -- ¿La task produjo código que hay que commitear? La setea el worker al
  -- terminar (--commitable / --no-commitable) o el humano desde la UI.
  --   NULL = sin declarar (tasks viejas o en curso)
  --   1    = tiene código → la toma "commit all"
  --   0    = informativa / sin código → la EXCLUYE "commit all"
  commitable     INTEGER,
...
export function migrate(db) {
  ...
  // ¿Tiene código para commitear? NULL = sin declarar, 1 = sí, 0 = no.
  // "commit all" solo toma las que no son 0 (ver requestCommitAllDone).
  add("task", "commitable", "commitable INTEGER");
  ...
  add("project", "pull_requested", "pull_requested INTEGER NOT NULL DEFAULT 0");
  add("project", "last_pull_at", "last_pull_at TEXT");
  add("project", "pull_status", "pull_status TEXT");
  ...
  // Proceso de promoción por destino: cómo el worker entrega el código a esa
  // rama (push directo / merge de otra rama / PR). Ver comentarios en la tabla.
  add("project_branch", "promote_strategy", "promote_strategy TEXT NOT NULL DEFAULT 'push'");
  add("project_branch", "promote_from", "promote_from TEXT");
  add("project_branch", "promote_notes", "promote_notes TEXT");
